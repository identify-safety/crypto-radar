...
信号:
  ...
  FUNDING_ANOMALY   watchlist 合约 fundingRate 年化 >15%
...
def scan_funding(insts):
    """watchlist 费率 → FUNDING_ANOMALY 候选 (年化>15%)"""
    out = []
    ...
    for inst in WATCHLIST:
        ...
        try:
            ...
            ann = fr * (24 / FUND_INTERVAL_H) * 365 * 100
            if ann > FUND_YEARLY_MIN:
                out.append((inst, fr, ann, row.get("fundingTime")))
        except Exception:
            continue
    return out
...
def sig_text(sig):
    ...
    if typ == "NEW_LISTING":
        return (f"[机会雷达 A级 {now_bj()}]\nOKX 新合约上线: {inst}\n"
                ...
                f"上线初期流动性浅, 先观察 1-2h 再决定(费率/溢价异动我会再推)")
    ...
    if typ == "FUNDING_ANOMALY":
        return (f"[机会雷达 A级 {now_bj()}]\nOKX 费率异动, 吃费率窗口:\n"
                f"{inst} {payload['fr']*100:+.4f}%/期 → 年化约 {payload['ann']:.0f}%\n"
...
async def amain():
    ...
    try:
        ...
        for inst, fr, ann, ft in fund:
            signals.append({"type": "FUNDING_ANOMALY", "inst": inst,
                            "sig_key": f"FUNDING_ANOMALY:{inst}:{ft}",
                            "payload": {"instId": inst, "fr": fr, "ann": ann, "fundingTime": ft}})
        ...
        if conn:
            await db_run(conn, True, len(insts), len(WATCHLIST), len(signals), None)
        print(f"OK scanned={len(insts)} signals={len(signals)} sent={n_sent}")
    except Exception as e:
